...
# sum_weight = reduce(lambda x, y: x+y, w)
# w = map(lambda val: val / sum_weight, w)
w_max = max(w)

#index = random pick from uniform set of all indicies (N)
index = random.randint(0, N - 1)
beta = 0
for n in range(N):
    # beta = beta + random pick from uniform set of 0 to 2*sum_weight
